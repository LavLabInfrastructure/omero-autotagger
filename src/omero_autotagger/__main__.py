"""Tags OMERO Images based on a set of rules."""

# really should refactor to make pylint happy instead of disabling the complaints
# but cannot be bothered right now
# pylint: disable=too-many-arguments, too-many-locals, too-many-branches, too-many-statements, too-many-instance-attributes, too-many-nested-blocks
...
class OmeroTagger:
    """
    Tags OMERO Images based on a set of rules defined in yaml."""
    ...
    def __init__(
        ...
        # blank tagmap
        self.tag_map: dict[str, omero.gateway.TagAnnotationWrapper] = {}

    def close(self) -> None:
        """Cleans up, currently just closes omero connection."""
        self.conn.close()
    ...
    def _get_method(self, obj, attr_name):
        ...
        for name in dir(obj_class):
            if (
                name.lower() == "get" + lower_case_attr_name
                or name.lower() == "list" + lower_case_attr_name
            ):
    ...
    def _validate_attr_path(self, path: list[str]):
        ...
        for i, obj in enumerate(path[:-1]):
            ...
            if obj.lower() == "parent":
                continue
            wrapper = omero.gateway.KNOWN_WRAPPERS.get(obj)
            if wrapper is None:
                logging.warning(
                    "Could not validate or invalidate path: %s Wrapper for object: %s does not exist! Continuing, but errors may occur!",  # pylint: disable=line-too-long
                    path,
                    obj,
                )
                break
            attr = path[i + 1]
            if (
                self._get_method(wrapper(), attr) is None
                and self._get_method(wrapper(), self.plural_noun(attr)) is None
            ):
                raise ValueError(
                    f"Could not find method to get: {attr} from: {wrapper.__name__}"
                )

    def compile_rules(self, tag_rules):
        """Compiles and validates rules defined in yaml."""
        ...
        for tag_rule in tag_rules:
            if "capture" in tag_rule:  # name based tagging
                if "name" in tag_rule:  # enforce exclusivity
                    logging.error(
                        'The "capture" and "name" attributes are incompatible. This is a typo or you are looking for "format"'  # pylint: disable=line-too-long
                    )
                    raise ValueError()
                ...
                try:  # check regex
                    ...
                    if compiled_regex.groups < 1:  # force capture regex
                        raise re.error("No capture groups!")
                except re.error as e:
                    logging.error(
                        "Error parsing yaml! Invalid capture regex: %s", capture_regex
                    )
                    raise e

                if "format" in tag_rule:  # check format
                    test = [""] * compiled_regex.groups
                    assert isinstance(tag_rule["format"], str)
                    try:
                        tag_rule["format"].format(test)
                    except IndexError as e:
                        logging.error(
                            "regex capture group gets a different number of variables than format expects. capture gets: %s format string: %s",  # pylint: disable=line-too-long
                            compiled_regex.groups,
                            tag_rule["format"],
                        )
                        raise e
                else:
            ...
            # attribute based tagging
                ...
                assert isinstance(tag_rule["name"], str)

                if "absolute" in tag_rule:  # default absolute rules (unset false tags)
                    assert isinstance(tag_rule["absolute"], bool)
                else:
                    tag_rule.update({"absolute": True})

                if "rules" not in tag_rule:  # check attribute tagging rules
                    logging.error("No rules provided! Tag: %s", tag_rule["name"])
                    raise ValueError()
                rules = tag_rule["rules"]
                assert isinstance(rules, list)
                for rule in rules:
                    if (
                        "attribute_path" not in rule
                        or "operation" not in rule
                        or "value" not in rule
                    ):
                        logging.error(
                            "Incomplete rule! path: %s, operation: %s, value: %s",
                            rule.get("attribute_path"),
                            rule.get("operation"),
                            rule.get("value"),
                        )
                        raise ValueError()
                    ...
                    if (
                        rule["operation"]
                        not in self.OPERATIONS.keys()  # pylint: disable=consider-iterating-dictionary
                    ):  # check operation
                        logging.error("Unknown operation: %s", rule["operation"])
                        raise ValueError()

                    path = rule[
                        "attribute_path"
                    ]  # check attribute path syntax TODO check wrappers for attributes
                    assert isinstance(path, list)
                    if len(path) < 2:
                        logging.error(
                            "Path should be a minumum of two items, the root item and the attribute"
                        )
                        raise ValueError()
            ...
            # invalid tagging rule type
                ...
                logging.error("Invalid tag rule! rule: %s", tag_rule)
                raise ValueError()
    ...
    def _apply_attr_rules(self, obj, obj_type, attr_rules, attr_names, path_tree):
        ...
        for attr_name in keys:
            # if last, is attribute, generate and use a getter
            # check rules for this attr and append valid tags
    ...
    def _create_tag(self, tag_val):
        tag = omero.gateway.TagAnnotationWrapper(self.conn)
    ...
    def _get_tag(self, tag_val):
        ...
        if tag is None:
            tag = list(
                self.conn.getObjects("TagAnnotation", attributes={"textValue": tag_val})
            )
            if not tag:
    ...
    def apply_rules(
        self, name_rules=None, attr_rules=None, attr_names=None, path_tree=None
    ):
        """Apply the tagging rules to the OMERO database."""
        ...
        for parent_obj_type in path_tree.keys():
            for parent_obj in self.conn.getObjects(parent_obj_type):
                ...
                attr_tagvals, remove_tags = self._apply_attr_rules(
                    parent_obj,
                    parent_obj_type,
                    attr_rules,
                    attr_names,
                    path_tree[parent_obj_type],
                )

                true_tag_vals, false_tag_vals = [
                    *set([*name_tagvals, *attr_tagvals])
                ], [*set(remove_tags)]
...
if __name__ == "__main__":
    ...
    def establish_connection(
        server, port, user=None, password=None, secure=False, session_key=None
    ):
        """Establishes a connection to the OMERO server."""
        if session_key is not None:
            ...
            conn = omero.gateway.BlitzGateway(
                user, password, host=server, port=port, secure=secure
            )
        ...
        if not connected:
            print(
                "Connection to OMERO server failed. Please check your settings and try again."
            )
            sys.exit(1)
    ...
    parser = argparse.ArgumentParser(
        description="A command-line interface for the OMERO tagging tool. This tool applies a set of tagging rules to an OMERO database, based on a given YAML file and patch script."  # pylint: disable=line-too-long
    )
    parser.add_argument(
        "tag_rules",
        type=str,
    ...
    parser.add_argument(
        "--patch", type=str, help="Path to the Python patch script to use for tagging."
    )
    parser.add_argument(
        "-s",
        "--server",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to establish a secure connection to the server. If not specified, a secure connection is used to login and is switched over to unsecure by default.",  # pylint: disable=line-too-long
    )
    parser.add_argument(
        "--session",
        type=str,
        help="Session key to use for connecting to the server. If this is provided, username and password are not required.",  # pylint: disable=line-too-long
    )
    parser.add_argument(
        "--dry",
        action="store_true",
        help="Perform a dry run of the autotagger displaying what tags will be added/removed. Output will be sent to csv file",  # pylint: disable=line-too-long
    )
    ...
    if dry:
        print(
            "This will be a dry run, the tags will not be committed.",
            "The output will be sent to a csv file.",
        )

    # Load tag rules from the YAML file
    with open(args.tag_rules, "r", encoding="utf-8") as file:
        tag_rules_yaml = yaml.safe_load(file)

    connection = establish_connection(
        args.server, args.port, args.user, args.password, args.secure, args.session
    )

    tagger = OmeroTagger(connection, tag_rules_yaml, args.patch, dry)
    tagger.apply_rules()
    tagger.close()
